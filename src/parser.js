...
/**
 * Parse <use partial="name" /> includes
 * Partials are simple file includes without props
 */
function parsePartials(source) {
  const results = [];
  const regex = /<use\s+partial="(.+?)"\s*\/>/g;
  let match;
  while ((match = regex.exec(source)) !== null) {
    results.push({ full: match[0], name: match[1] });
  }
  return results;
}

/**
 * Parse {{#with variable}} ... {{/with}} blocks
 * Scopes variables to a sub-object
 */
function parseWithBlocks(source) {
  const results = [];
  const regex = /\{\{#with\s+(\w+)\}\}([\s\S]*?)\{\{\/with\}\}/g;
  let match;
  while ((match = regex.exec(source)) !== null) {
    results.push({
      full: match[0],
...
/**
 * Parse {{ variable | filter }} pipe syntax
 * Returns array of { full, variable, filters }
 */
function parseFilters(source) {
  ...
  let match;
  while ((match = regex.exec(source)) !== null) {
    const filters = match[2].split("|").map((f) => f.trim());
    results.push({
      full: match[0],
      variable: match[1],
      filters,
...
/**
 * Parse :class directives for conditional CSS classes
 * <div :class="{ active: isActive, hidden: isHidden }">
 */
function parseClassDirectives(source) {
  const results = [];
  const regex = /:class="(\{[^"]*\})"/g;
  let match;
  while ((match = regex.exec(source)) !== null) {
    results.push({ full: `:class="${match[1]}"`, expression: match[1] });
  }
  return results;
}

/**
 * Parse <markdown> ... </markdown> blocks
 */
function parseMarkdownBlocks(source) {
  ...
  let match;
  while ((match = regex.exec(source)) !== null) {
    results.push({ full: match[0], content: match[1].trim() });
  }
  return results;
}
